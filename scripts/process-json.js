...
function processJSON() {
  ...
  console.log(`총 ${records.length}개 레코드 로드됨`);
  ...
  for (const record of records) {
    ...
    if (processedCount % 100000 === 0) {
      console.log(`${processedCount}개 레코드 처리 중...`);
    }
    ...
    // 유닛 정보 처리
    const unitsInRecord = [];
    const unitsInfo = record.units_info || [];

    for (let slot = 0; slot < unitsInfo.length && slot < 5; slot++) {
      const unitInfo = unitsInfo[slot];
      const unitType = unitInfo?.unit_type;

      if (unitType !== undefined && unitType !== null && !isNaN(parseInt(unitType))) {
  ...
  console.log(`총 ${processedCount}개 레코드 처리 완료`);
  return stats;
}
